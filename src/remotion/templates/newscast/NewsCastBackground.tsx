...
import { AbsoluteFill, interpolate, useCurrentFrame, useVideoConfig, delayRender, continueRender } from "remotion";
...
export const NewsCastBackground: React.FC<{
  ...
  const [topoHandle] = useState(() => delayRender("newscast-world-topology", { timeoutInMilliseconds: 10_000 }));
  ...
  useEffect(() => {
    fetch(WORLD_TOPOLOGY_URL)
      ...
      .then((data) => {
        setTopology(data);
        continueRender(topoHandle);
      })
      .catch(() => continueRender(topoHandle));
  }, [topoHandle]);
  ...
  if (variant === "pixel_map") {
    ...
    return (
      <AbsoluteFill style={{ backgroundColor: solidBackground ? DEFAULT_BG : "transparent", overflow: "hidden" }}>
        ...
        />
        {/* @ts-expect-error react-simple-maps JSX type mismatch with React 18 */}
        <ComposableMap
